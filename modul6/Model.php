<?php
    ...
    function GetDataPeminjaman($pdo)
    {
    ...
    function TambahPeminjaman($pdo, $tgl_pinjam, $tgl_kembali) {
    ...
    function HapusPeminjaman($pdo, $id) {
    ...
    function TambahMember($pdo, $nama_member, $nomor_member, $password_member, $alamat, $tgl_mendaftar, $tgl_terakhir_bayar) {
        $sql = "INSERT INTO member (nama_member, nomor_member, password_member, alamat, tgl_mendaftar, tgl_terakhir_bayar) VALUES (?,?,?,?,?,?)";
        $stmt= $pdo->prepare($sql);
        $stmt->execute([$nama_member, $nomor_member, $password_member, $alamat, $tgl_mendaftar, $tgl_terakhir_bayar]);
        $pdo = null;
    ...
    function UpdateMember($pdo, $nama_member, $nomor_member, $password_member, $alamat, $tgl_mendaftar, $tgl_terakhir_bayar, $id){
        $sql = "UPDATE member SET nama_member=?, nomor_member=?, password_member=?, alamat=?, tgl_mendaftar=?, tgl_terakhir_bayar=? WHERE id_member=?";
        $stmt= $pdo->prepare($sql);
        $stmt->execute([$nama_member, $nomor_member, $password_member, $alamat, $tgl_mendaftar, $tgl_terakhir_bayar, $id]);
        $pdo = null;
        header('Location: Member.php');
        exit();
    }

    function HapusMember($pdo, $id) {
    ...
    function TambahBuku($pdo, $judul_buku, $penulis, $penerbit, $tahun_terbit) {
    ...
    function CekUserSession($session)
    {
        if ($session == "Guest")
